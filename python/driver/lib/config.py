...
if(os.path.exists("/run/secrets/ARBITER_TOKEN")):
    ARBITER_TOKEN = (open("/run/secrets/ARBITER_TOKEN", "rb").read()).decode("utf-8", errors="ignore")
    ...
    ARBITER_TOKEN = (open("/run/secrets/CM_KEY", "rb").read()).decode("utf-8", errors="ignore")
    #arbiterTokenbase64 = base64.b64encode(arbiterToken)
else:
